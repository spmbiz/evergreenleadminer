...
QUALITY_VERSION = "HOSPITALITY_COMMERCIAL_FIT_V2_3"
...
STR=(r"\bvacation rentals?\b",r"\bvacation homes?\b",r"\bholiday rentals?\b",r"\bholiday homes?\b",r"\bshort[- ]term rentals?\b",r"\bshort stays?\b",r"\bnightly rentals?\b",r"\bserviced apartments?\b",r"\bserviced accommodation\b",r"\baparthotel\b",r"\bvilla rentals?\b",r"\bchalet rentals?\b",r"\bcabin rentals?\b",r"\bself[- ]catering\b",r"\bdirect bookings?\b")
PROPERTY=(r"\bhotels?\b",r"\bresorts?\b",r"\bvillas?\b",r"\bchalets?\b",r"\bholiday homes?\b",r"\bvacation homes?\b",r"\baparthotel\b",r"\bserviced apartments?\b",r"\bcottages?\b",r"\bcabins?\b",r"\blodges?\b")
...
WEAK={"PENSION_GUESTHOUSE":(r"\bpension\b",r"\bgaestehaus\b",r"\bgastehaus\b",r"\bguest ?house\b"),"ORDINARY_INN":(r"\blandgasthof\b",r"\bgasthof\b",r"\bgasthaus\b",r"\binn\b"),"BUDGET_LODGING":(r"\bmotel\b",r"\bbudget hotel\b",r"\beconomy hotel\b",r"\bbed\s*(?:and|&)\s*breakfast\b",r"\bb\s*&\s*b\b")}
PREMIUM=((r"\b5[- ]star\b|\bfive[- ]star\b|\*\*\*\*\*",25,"5_STAR"),(r"\b4[- ]star superior\b|\bfour[- ]star superior\b|\*\*\*\*\s*superior",22,"4_STAR_SUPERIOR"),(r"\b4[- ]star\b|\bfour[- ]star\b|\*\*\*\*",14,"4_STAR"),(r"\bluxur(?:y|ious)\b|\bexclusive\b|\bpremium\b",13,"LUXURY"),(r"\bboutique hotel\b|\bdesign hotel\b|\bboutique resort\b",11,"BOUTIQUE_DESIGN"),(r"\bwellness\b|\bspa\b",10,"SPA_WELLNESS"),(r"\binfinity pool\b|\bprivate pool\b|\bpool villa\b",8,"PREMIUM_POOL"),(r"\bsuites?\b|\bpenthouse\b",6,"SUITES"),(r"\bmichelin\b|\bfine dining\b|\bgourmet\b",8,"GASTRONOMY"),(r"\badults[- ]only\b",5,"ADULTS_ONLY"),(r"\bpalace\b|\bchateau\b|\bcastle hotel\b|\bcountry estate\b",8,"PRESTIGE_PROPERTY"))

def txt(v):
 s=unicodedata.normalize("NFKD",str(v or "")).encode("ascii","ignore").decode().lower().replace("_"," ").replace("&amp;"," and ");return re.sub(r"\s+"," ",s).strip()
def host(v):
 s=str(v or "").strip()
 if not s:return ""
...
def assess_record(row,page_text="",final_url=""):
 ...
 sh=hits(ev,STR);ph=hits(ev,PROPERTY);lh=hits(ev,SELF_LODGING);dh=hits(ev,DESTINATION);generic_pm=anyhit(ev,GENERIC_PM);hard=""
 ...
 if generic_pm and sh==0:
  ...
  else:pm_hold=True;reasons.append("GENERIC_PROPERTY_MANAGEMENT_NEEDS_LIVE_STR_REVALIDATION")
 ...
 primary=name+" "+category;target_name=bool(re.search(r"\b(?:vacation|holiday) homes?\b|\bholiday cottages?\b|\bcottages?\b|\bcabins?\b|\bvillas?\b|\bself[- ]catering\b|\brentals?\b",name))
 if re.search(r"\b(?:campground|camping|caravan park|rv park)\b",primary) and "glamping" not in primary and sh==0 and lh==0 and not target_name:hard=hard or "NON_TARGET_CAMPGROUND";reasons.append("NON_TARGET_CAMPGROUND")
 restaurant_primary=bool(re.search(r"\b(?:restaurant|cafe|bar)\b",category) or re.match(r"^(?:the\s+)?(?:restaurant|cafe)\b",name))
 ...
 entity=max(0,min(100,entity));premium=25 if ph else 10;signals=[]
 ...
 premium=max(0,min(100,premium));operator=20+min(55,hits(ev,OPERATOR)*18)
 ...
 elif pm_hold:tier,ready,decision="C",False,"REVIEW"
