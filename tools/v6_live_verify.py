...
When an official website links to an Instagram profile, the public profile URL is
captured as enrichment evidence for the canonical MASTER Instagram field.
"""
...
UA="Mozilla/5.0 (compatible; AIProdLeadVerifier/1.1; public-business-research)"
...
def extract_instagram(raw_html,base_url):
    """Return an Instagram profile linked directly from the official site, never inferred."""
    for href in re.findall(r"href\s*=\s*[\"']([^\"']+)[\"']",raw_html or "",flags=re.I):
        try:
            u=urljoin(base_url,htmlmod.unescape(href).strip())
            p=urlparse(u); h=(p.hostname or "").lower().strip(".")
...
def verify(row,timeout):
    ...
    try:
        ...
        if ig:
            out["instagram"]=ig; out["instagram_source_url"]=r.url
