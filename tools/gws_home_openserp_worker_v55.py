...
def request_json(url,timeout=25):
    req=urllib.request.Request(url,headers={"User-Agent":"GWS-Home-OpenSERP/5.5","Accept":"application/json"}); z=time.time()
    try:
...
def direct_fetch(c,url):
    ...
    try:
        with urllib.request.urlopen(req,timeout=12) as r:
            ...
            return {"seed":url,"final":final,"status":int(r.status),"ok":True,"identity":ide,"dns_negative":False}
        ...
        return {"seed":url,"final":url,"status":404 if dns else 0,"ok":False,"dns_negative":dns,"error":"" if dns else type(e).__name__,"error_detail":str(e)[:180]}
    except Exception as e:
        return {"seed":url,"final":url,"status":getattr(e,"code",0) or 0,"ok":False,"dns_negative":False,"error":type(e).__name__,"error_detail":str(e)[:180]}
...
def run_pass(c,endpoint,engines,pass_no,sleep_min,sleep_max):
    queries=list(v5.query_set(c,pass_no))[:2]; search_health=[]; healthy=set(); usable=0; serp_candidates=[]; seen=set()
    for q in queries:
        qh=[]; fam=set()
        ...
        search_health.append({"query":q,"providers":qh,"parsed_families":sorted(fam),"external_domains":len({x["host"] for x in serp_candidates})})
    direct_health=[]; owned=""; owned_identity={}; owned_via=""
    for u in seeds_for_pass(c,pass_no)[:12]:
        de=direct_fetch(c,u); direct_health.append(de)
        if de.get("ok") and (de.get("identity") or {}).get("matched") and not v2.platform(de.get("final") or u):
            owned=de.get("final") or u; owned_identity=de.get("identity") or {}; owned_via="residential_direct_lattice"; break
    if not owned:
        checked_hosts={v2.host(x.get("final") or x.get("seed") or "") for x in direct_health}
        for item in serp_candidates:
            if item["host"] in checked_hosts: continue
            yes,why=plausible(c,item)
            if not yes: continue
            de=direct_fetch(c,item["url"]); de["serp_hint"]=why; direct_health.append(de); checked_hosts.add(item["host"])
            if de.get("ok") and (de.get("identity") or {}).get("matched") and not v2.platform(de.get("final") or item["url"]):
                owned=de.get("final") or item["url"]; owned_identity=de.get("identity") or {}; owned_via="residential_serp"; break
            if len(direct_health)>=20: break
    return {"search_queries":len(queries),"search_usable_queries":usable,"search_health":search_health,"search_candidates":[x["url"] for x in serp_candidates],"healthy_providers":sorted(healthy),"direct_checked":len(direct_health),"direct_health":direct_health,"owned":owned,"owned_identity":owned_identity,"owned_via":owned_via,"candidate_seeds":seeds_for_pass(c,pass_no),"residential_pass":pass_no}
...
def main():
    ...
    for i,row in enumerate(rows,1):
        c=row["candidate"]; pe=row["place"]; p1=run_pass(c,endpoint,engines,1,a.sleep_min,a.sleep_max); p2=run_pass(c,endpoint,engines,2,a.sleep_min,a.sleep_max)
        for p in (p1,p2):
        ...
        rec={"schema":"gws-home-openserp-observation-v2","r":row["r"],"candidate":c,"place":pe,"pass1":p1,"pass2":p2,"certificate":cert,"owned_site":owned or "","status":status,"reason":reason,"certificate_eligible":bool(cert.get("verified"))}
