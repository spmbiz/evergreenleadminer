...
def http_version(url: str) -> tuple[str, dict]:
    ...
    with urllib.request.urlopen(req, timeout=15) as response:
        ...
        return version, {"etag":etag, "last_modified":modified, "content_length":length, "final_url":response.geturl()}
...
def atp_tasks(now, coverage):
    ...
    for item in cfg.get("spiders") or []:
        ...
        tasks.append({
            ...
            "release":release,"source_final_url":final_url,"spider":spider,"extract_id":"",
...
def osm_tasks(now, coverage):
    ...
    for item in cfg.get("extracts") or []:
        ...
        tasks.append({
            ...
            "release":release,"source_final_url":headers.get("final_url") or pbf_url,"spider":"","extract_id":extract_id,
...
def main():
    ...
    source_due = atp + osm
    source_due.sort(key=lambda x:(bool(x.get("source_changed")), int(x.get("priority") or 0), float(x.get("source_age_hours") or 0), x["name"]), reverse=True)
    total_source_cap = min(4, max(0, atp_cap) + max(0, osm_cap))
    ...
    if force in ("atp", "alltheplaces", "atp_directory_contact"):
        ...
        total_source_cap = int(a.capacity)
    ...
    source_only_force = force in ("atp", "alltheplaces", "atp_directory_contact", "osm", "geofabrik", "osm_geofabrik")
    ...
    payload = dict(geo)
    payload.update({
        "capacity":int(a.capacity),"geo_capacity":geo_capacity,"source_slot_cap":total_source_cap,
        "external_source_backlog":len(atp)+len(osm),"source_errors":atp_errors+osm_errors,
        "useful_backlog":int(geo.get("useful_backlog") or 0)+len(atp)+len(osm),
        "lane_backlog":lane_backlog,"selected_lane_counts":selected_lane_counts,"include":selected,
    })
    out = Path(a.out)
